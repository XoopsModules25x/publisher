...
if ($op === 'start') {
    ...
    PublisherUtilities::openCollapsableBar('amsimport', 'amsimporticon', sprintf(_AM_PUBLISHER_IMPORT_FROM, $importFromModuleName), _AM_PUBLISHER_IMPORT_INFO);
    ...
    if ($totalCat == 0) {
        ...
        if ($totalArticles == 0) {
            ...
            $sql = 'SELECT cat.topic_id, cat.topic_pid, cat.topic_title, COUNT(art.storyid) FROM ' . $GLOBALS['xoopsDB']->prefix('ams_topics') . ' AS cat INNER JOIN '
    ...
    PublisherUtilities::closeCollapsableBar('amsimport', 'amsimporticon');
    xoops_cp_footer();
}
...
if ($op === 'go') {
    ...
    PublisherUtilities::openCollapsableBar('amsimportgo', 'amsimportgoicon', sprintf(_AM_PUBLISHER_IMPORT_FROM, $importFromModuleName), _AM_PUBLISHER_IMPORT_RESULT);
  ...
  //                $image_display = empty($image_display) ? 0 : 1;
    ...
    PublisherUtilities::closeCollapsableBar('amsimportgo', 'amsimportgoicon');
    xoops_cp_footer();
}
