...
if ($op == 'go') {
    ...
    while ($arrCat = $xoopsDB->fetchArray($resultCat)) {
        ...
        if (($arrCat['topic_imgurl'] != 'blank.gif') && ($arrCat['topic_imgurl'] != '')) {
            if (copy(XOOPS_ROOT_PATH . "/modules/AMS/images/topics/" . $arrCat['topic_imgurl'], XOOPS_ROOT_PATH . "/uploads/publisher/images/category/" . $arrCat['topic_imgurl'])) {
        ...
        $groupsIds = $gperm_handler->getGroupIds('ams_view', $arrCat['topic_id'], $ams_module_id);
        publisher_saveCategoryPermissions($groupsIds, $categoryObj->categoryid(), 'category_read');
        $groupsIds = $gperm_handler->getGroupIds('ams_submit', $arrCat['topic_id'], $ams_module_id);
