...
<{if $block.display_type=='block'}>
    ...
    <{foreach item=item from=$block.items}>        <{include file="db:publisher_singleitem_block.tpl" item=$item}>    <{/foreach}>
...
<{else}>    <{foreach item=item from=$block.items name=spotlight}>        <{if $item.summary != ''}>
    <div class="itemText" style="padding-left: 5px; padding-top: 5px;">
        <div>
            <img style="vertical-align: middle;" src="<{$block.moduleUrl}>/assets/images/links/doc.png" alt=""/>&nbsp;<{$item.titlelink}>
        </div>

        <div>
            <{if $item.image_path}>
                <img class="publisher_item_image" src="<{$item.image_path}>" align="right" alt="<{$item.clean_title}>" title="<{$item.clean_title}>"/>
            <{/if}> <{$item.summary}>
        </div>
    </div>
