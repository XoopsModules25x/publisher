...
class PublisherUtilities
{
    ...
    public static function editCategory($showmenu = false, $categoryId = 0, $nbSubCats = 4, $categoryObj = null)
    {
        ...
        if ($categoryId) {
            ...
            echo "<input type='button' name='button' onclick=\"location='item.php?op=mod&categoryid="
                 . $selCat
                 . "'\" value='"
                 . _AM_PUBLISHER_CREATEITEM
                 . "'>&nbsp;&nbsp;";
            echo '</div>';
        }
        //end of fx2024 code
    ...
    public static function &getPathStatus($item, $getStatus = false)
    {
        ...
        if (is_writable($thePath)) {
            ...
            $pathCheckResult = -1;
            $pathStatus      = _AM_PUBLISHER_NOTAVAILABLE
                               . " <a href='"
                               . PUBLISHER_ADMIN_URL
                               . "/index.php?op=createdir&amp;path={$item}'>"
                               . _AM_PUBLISHER_CREATETHEDIR
                               . '</a>';
        } else {
            $pathCheckResult = -2;
            $pathStatus      = _AM_PUBLISHER_NOTWRITABLE
                               . " <a href='"
                               . PUBLISHER_ADMIN_URL
                               . "/index.php?op=setperm&amp;path={$item}'>"
                               . _AM_PUBLISHER_SETMPERM
                               . '</a>';
        }
        if (!$getStatus) {
            return $pathStatus;
    ...
    public static function openCollapsableBar($tablename = '', $iconname = '', $tabletitle = '', $tabledsc = '', $open = true)
    {
        ...
        echo "<h3 style=\"color: #2F5376; font-weight: bold; font-size: 14px; margin: 6px 0 0 0; \"><a href='javascript:;' onclick=\"toggle('"
             . $tablename
             . "'); toggleIcon('"
             . $iconname
             . "')\">";
            ...
            foreach ($errArray as $key => $error) {
                if (is_array($error)) {
                    foreach ($error as $err) {
                        echo '<li><a href="#'
                             . $key
                             . '" onclick="var e = xoopsGetElementById(\''
                             . $key
                             . '\'); e.focus();">'
    ...
    public static function uploadFile($another = false, $withRedirect = true, &$itemObj)
    {
        ...
        if (!$fileObj->store($allowedMimetypes)) {
            ...
            try {
                ...
                redirect_header('file.php?op=mod&itemid=' . $fileObj->itemid(), 3,
                                _CO_PUBLISHER_FILEUPLOAD_ERROR . static::formatErrors($fileObj->getErrors()));
            }
            //    } else {
    ...
    public static function ratingBar($itemId)
    {
        ...
        if (!$gpermHandler->checkRight('global', PublisherConstants::PUBLISHER_RATE, $groups, $publisher->getModule()->getVar('mid'))) {
            ...
            $staticRater[] .= '<div class="publisher_current-rating" style="width:'
                              . $ratingWidth
                              . 'px;">'
            ...
            $rater .= '<div class="publisher_current-rating" style="width:'
                      . $ratingWidth
                      . 'px;">'
                      . _MD_PUBLISHER_VOTE_RATING
                      . ' '
