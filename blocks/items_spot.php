    ...
    if ($optDisplayLast == 1) {
        ...
        if ($itemsObj) {
            if ($optCategoryId != -1 && $optCatImage) {
                ...
                if ($cat->getImage() !== 'blank.png') {
                    $category['image_path'] = PublisherUtilities::getImageDir('category', false) . $cat->getImage();
                } else {
                    $category['image_path'] = '';
                }
        ...
        foreach ($selItems as $itemId) {
            ...
            if (!$itemObj->notLoaded()) {
                ...
                if ($optTruncate > 0) {
                    $block['truncate'] = true;
                    $item['summary']   = PublisherUtilities::truncateTagSafe($item['summary'], $optTruncate);
                }
                $block['items'][] = $item;
                ++$i;
            }
...
function publisher_items_spot_edit($options)
{
    ...
    $catEle    = new XoopsFormLabel(_MB_PUBLISHER_SELECTCAT, PublisherUtilities::createCategorySelect($options[2], 0, true, 'options[2]'));
    $publisher = PublisherPublisher::getInstance();
