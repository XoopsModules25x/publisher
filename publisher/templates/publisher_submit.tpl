...
<{if $op == 'preview'}>
<br/>    <{include file="db:publisher_singleitem.tpl" item=$item}><{/if}>
...
<div id="tabs">
    ...
    <{$form.extra}>><!-- start of form elements loop --><{foreach item=tab key=key from=$form.tabs}>
    <div id="tab_<{$key}>">
        <table class="outer" cellspacing="1">
            <{foreach item=element from=$form.elements}> <{if $element.tab == $key || $element.tab == -1}> <{if !$element.hidden}>
            <tr>
            ...
            <{/if}> <{/if}> <{/foreach}><!-- end of form elements loop -->
        </table>
    </div>
    <{/foreach}> <{foreach item=element from=$form.elements}> <{if $element.hidden}><{$element.body}><{/if}> <{/foreach}></form>
</div>
